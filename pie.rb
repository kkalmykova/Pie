...
class Pie
  attr_reader :area, :raisin, :height, :width
  ...
  def possible_widths
    if self.area % self.raisin != 0
      return []
    end

    widths = ((self.area / self.raisin) / (self.height + 1) + 1..self.width).to_a.select { |i| (self.area / self.raisin) % i == 0 }
    widths.reverse
  end
...
class Slice
  attr_reader :pie, :x, :y, :area, :raisin, :width, :height
  ...
  def initialize(pie, x, y, width)
    ...
    # Check If the cake can be cut into equal slices
    @area = pie.area % pie.raisin == 0 ?
              pie.area / pie.raisin : nil
    if @area == nil || width <= 0
      return
    end
...
class Slices
  ...
  # Checks if slices consist only one raisin
  def only_one_raisin?()
...
#check if everything valid, all slices consist only one raisin, if it overlap, check slice by slice
def cut pie
  ...
  while done.length > 0 && done.length <= pie.raisin
    if done.length == pie.raisin
      if done.valid? && done.only_one_raisin? && !done.overlap? && done.free_dotes.nil?
      ...
      if done.valid? && done.only_one_raisin? && !done.overlap?
